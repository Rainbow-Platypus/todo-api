...
const r = db.prepare('INSERT INTO todos (title) VALUES
(?)').run(req.body.title);
res.status(201).json({ id: r.lastInsertRowid, title: req.body.title, done:
0 });
});
...
WHERE id = ?'
).run(req.body.title || null, req.body.done != null ? (req.body.done ? 1 :
0) : null, req.params.id);
if (r.changes === 0) return res.sendStatus(404);
res.json(db.prepare('SELECT * FROM todos WHERE id =
?').get(req.params.id));
});
